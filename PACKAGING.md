...
A package version such as `1.0.16` produces package and informational version `1.0.16`, assembly and file version `1.0.16.0`, and exact Client and Server dependencies on `NetSquare.Core` version `[1.0.16]`.
...
.\Pack-NetSquare.ps1 `
    -Version 1.0.16 `
    -Obfuscate `
    -OutputDirectory .\Build\NuGet\1.0.16
```
...
```powershell
git tag -a v1.0.16 -m "NetSquare 1.0.16"
```
...
$directory = ".\Build\NuGet\1.0.16"

.\nuget6.exe push "$directory\NetSquare.Core.1.0.16.nupkg" `
    -Source $source -ApiKey $nugetApiKey -NonInteractive

.\nuget6.exe push "$directory\NetSquare.Client.1.0.16.nupkg" `
    -Source $source -ApiKey $nugetApiKey -NonInteractive

.\nuget6.exe push "$directory\NetSquare.Server.1.0.16.nupkg" `
    -Source $source -ApiKey $nugetApiKey -NonInteractive
```
