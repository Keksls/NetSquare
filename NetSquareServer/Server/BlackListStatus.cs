...
{
    /// <summary>
    /// Describes the current hit, escalation and ban state of a generic subject.
    /// </summary>
    public sealed class BlackListStatus
    {
        ...
        public DisconnectInfo CreateDisconnectInfo()
        {
            if (!IsBanned || !BanType.HasValue)
                throw new InvalidOperationException("The blacklist status does not contain an active local ban.");
